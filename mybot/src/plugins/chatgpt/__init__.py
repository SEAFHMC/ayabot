...
from nonebot.adapters.kaiheila.event import PrivateMessageEvent, MessageEvent
...
async def _(event: MessageEvent, msg: Message = CommandArg()):
    ...
    # 检测是否填写 API key
    if api_key == "":
        await chat_record.finish("请先配置openai_api_key")
...
async def _(event: MessageEvent, msg: Message = CommandArg()):
    ...
    content = msg.extract_plain_text()
    if content == "" or content is None:
        await chat_request.finish("内容不能为空！")
...
async def get_response(content, proxy):
    ...
    res = res_.choices[0].message.content  # type: ignore
